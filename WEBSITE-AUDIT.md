...
**Weaknesses:** ~~Nav duplication (C4)~~ **fixed in Batch 1**; ~~condition cards all link to `#book` instead of `/conditions/#anchor`~~ **fixed in Batch 2** — 7 of 10 cards (those with a matching section on `/conditions/`) now link to their anchor; Food Allergies, Lupus/RA, and Allergic Asthma have no matching section on the Conditions page yet, so they link to `/conditions/` rather than inventing new clinical copy — full list alignment (adding those 3 sections, or removing the cards) is a content decision for the clinic, still open; blog teaser cards all link to `/blog/` generically with a fake ▶ play affordance (still open — Roadmap item 8); ~~footer "Conditions" links go to homepage anchors, not the conditions page~~ **fixed in Batch 1**, now link to `/conditions/#anchor`; ~~no favicon link~~ **fixed in Batch 1**; ~~no og:image~~ **fixed in Batch 2** — tag added, file (`assets/images/home-og.jpg`) still needs to be supplied by the clinic; the comparison-table intro sentence ("The question above is one patients ask AI tools…") is written for search engines, not humans (still open); ~~testimonials contain precise clinical claims~~ **fixed in Batch 2** — softened to experience-focused language, inline "results vary" note added near the section (footer disclaimer also still present).
...
| Geo marquee | 🟢 KEEP |
| Conditions grid | ✅ Fixed (Batch 2) — 7/10 cards link to `/conditions/#anchor`; 3 without a matching section link to `/conditions/` pending a content decision |
| Journey timeline | 🟢 KEEP |
| Compare table | 🟡 IMPROVE — rewrite intro copy for humans |
| Testimonials | ✅ Fixed (Batch 2) — softened clinical specifics, added inline "results vary" note |
...
| og:image | ✅ Fixed (Batch 2) — tag added, file pending from clinic |
...
| 7 | **Homepage condition cards → `/conditions/#anchor`** instead of `#book`; align the condition lists between the two pages | index, conditions | Educate → trust → book, in that order; also builds internal-link relevance | ✅ **Done — Batch 2** (routing only; see note below) |
...
| 9 | **Add favicon links + og:image to Home/Team/Blog**; add team.html to sitemap | index, team, blog/index, sitemap | Basic SEO hygiene; social shares currently render blank | ✅ **Done — Batch 2** (og:image tags added to Home/Team; files still need to be supplied — see Assets). favicon links done in Batch 1. team.html was already in sitemap (original finding was inaccurate). Blog hub's og:image still open (tracked under item 13, blog hub retitle). |
| 10 | **Soften clinical claims in testimonials** + inline "results vary" note | index | Healthcare compliance and credibility with the educated-patient audience | ✅ **Done — Batch 2** |
...
2. **Batch 2 (Conversion & remaining trust items):** items 7, 9 (og:image), 10 — condition cards route to `/conditions/` before booking, social preview images, softened testimonial claims. ✅ **Shipped** — see [Phase 4 — Implementation Log](#phase-4--implementation-log).
...
## Batch 2 — Conversion & remaining trust items ✅ Shipped

**Roadmap items closed:** 7, 9 (remainder), 10
**Files changed:** `index.html`, `team.html`, `README.md`, `PUBLISHING-GUIDE.md`

**Summary:** All copy/link/meta-tag edits — no layout, design, or new assets. Homepage condition cards now route to `/conditions/#anchor` for the 7 conditions that have a matching section on the Conditions page (MCAS, Hashimoto's, Chronic Urticaria, Eczema, Allergic Rhinitis, Eosinophilic Esophagitis, POTS); the 3 without a matching section (Food Allergies, Lupus/RA, Allergic Asthma) now link to `/conditions/` rather than `#book` — no new clinical copy was invented to fill those gaps, since that's a business/content decision, not a technical one. `og:image` tags added to Home and Team, pointing at `assets/images/home-og.jpg` and `assets/images/team-og.jpg` respectively; both filenames added to the asset checklists in `README.md` and `PUBLISHING-GUIDE.md` so the clinic knows to supply them (files don't exist yet — same status as every other missing image asset, tracked under Roadmap item 11). Testimonials on the homepage had their precise, unverifiable clinical figures (exact antibody lab values, absolute "completely off medication" / "gone" language) softened to experience-focused phrasing without changing the underlying story; a visible "individual results vary" note was added directly under the testimonials grid (the existing footer disclaimer remains, this is now reinforced at the point of reading).

**Deferred / explicitly not done in this batch** (left for later batches per scope):
- Full condition-list alignment between Home and Conditions (adding the 3 missing condition write-ups, or removing those cards) — content decision, not made unilaterally.
- The three `home-og.jpg` / `team-og.jpg` / `online-consultation-og.jpg` image files themselves — blocked on the clinic supplying real assets (item 11).
- Blog hub's own og:image and retitle — item 13, Batch 3.

## Batch 3 — Legal & Compliance pages ✅ Shipped

**Roadmap item closed:** Legal Pages (`docs/24-ROADMAP.md` "Remaining", `docs/CHANGELOG.md` "Next Batch" item 1; also the top recommendation of `docs/20-PRODUCT-ARCHITECTURE-REVIEW.md` §7)
**Files changed:** New — `privacy.html`, `terms.html`, `disclaimer.html`. Modified — `index.html`, `team.html`, `contact.html`, `conditions/index.html`, `online-consultation/index.html`, `blog/index.html`, `gallery.html` (footer "Legal" section added to all 7); `online-consultation/index.html` also had a pre-existing invalid-markup bug fixed (stray `</body></html>` before the mobile-menu `<script>` block — harmless to browsers but invalid HTML, discovered while verifying this batch's footer edit did not break the file) and its footer "Book Consultation" link corrected from `wa.me` to `/contact.html` (an inconsistency Batch 1's changelog claimed was already fixed sitewide but had missed this one footer instance).

**Summary:** Added Privacy Policy, Terms of Use, and Medical Disclaimer as new standalone pages, reusing the existing "simple" header/footer/mobile-menu system (same pattern as `contact.html`) — no new component, no visual redesign. Content is written specifically for Wise Homeopathy using only facts already verifiable in the repository (exact form fields collected, Netlify as the forms processor, Google Fonts/Google Maps as the only third-party requests, no analytics/tracking present, existing NAP/contact details) — no invented certifications, retention periods, grievance-officer titles, or regulatory claims. Each page marks genuinely-future functionality (My Health Journey, Patient Login, WiseOS) with an explicit "Future Update Required" callout rather than describing it as if it exists. All 9 other public pages now link to the three new pages from a "Legal" footer section; `thanks.html` and `booking-received.html` (transient, `noindex` confirmation cards with no footer nav) were intentionally left unchanged.

**Cookie Policy decision:** not created as a separate page. The site runs no analytics or tracking cookies and has only two third-party embeds (Google Fonts, Google Maps); a short "Cookies & third-party content" section inside `privacy.html` covers this fully. A standalone Cookie Policy would overstate the complexity of what's actually happening.

**Verification performed:** HTML tag-balance check (`<html>`/`</html>`, `<body>`/`</body>`) across all 10 touched files — all balanced. Grep sweep confirmed every new page's internal links resolve to real files, and no page other than `thanks.html`/`booking-received.html` is missing the new Legal footer section.

**Not done in this batch** (explicitly out of scope, not silently invented): a formal retention period, a named grievance officer, any specific regulatory-compliance claim (e.g. DPDP Act 2023 mechanics), and any Cookie Policy content beyond what's in Privacy Policy — see the batch's final review answer in conversation for what's deferred to before Phase 2.
