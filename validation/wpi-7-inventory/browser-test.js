...
async function main() {
  ...
  try {
    ...
    // ---- 5. Pure-function unit check: inventoryHtml() against window.WiseDoctorDashboard ----
    {
      ...
      // Updated at Batch WPI-8 (docs/50 §11/§19): the hand-ported registry
      // now carries a fourth entry, `pillfill_orders` — a mechanical,
      // disclosed update to this assertion's own stale count, mirroring the
      // same update validation/wpi-4-doctor-dashboard/browser-test.js and
      // validation/wpi-5-appointment/browser-test.js already received in
      // this same change.
