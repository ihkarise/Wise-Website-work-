...
including the content-based MIME-spoofing rejection, the Drive-sharing-privacy
assertion, the partial-write rollback, and the cross-patient isolation checks on
list/download (the platform's highest-risk feature), is
...
detection or `DriveApp`'s own real sharing behavior at all — that is exclusively
`validation/phase-2a-foundation/`'s Node-mocked coverage, disclosed there as an
approximation of the real platform behavior, still not independently verified against a
live deployment per `docs/42-REPORTS-UPLOAD-READINESS-REVIEW.md` §11/§12), nor anything
