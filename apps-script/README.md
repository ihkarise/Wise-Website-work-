...
**Batch 4A** — schema, validation, and Sheet-write layer only.
No AI summarization, no email sending, no staff-facing form yet.
...
- **4B (staff entry form)** — a new static HTML page that POSTs the
  shape in `sample-payloads/valid-submission.json` to this Web App's
  URL. No changes needed here beyond deploying it.
...
4. Only after all of the above pass should this be pointed at a real
   Sheet — and even then, no email or AI step exists yet in this batch,
   so there is no patient-facing risk regardless.

## Explicitly out of scope for this batch

No AI summarization, no email sending, no staff-facing HTML form, no
