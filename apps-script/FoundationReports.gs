 ...
 * (docs/42-REPORTS-UPLOAD-READINESS-REVIEW.md), approved before any code
 * was written.
 *
 ...
 * exactly as both were already designed to be reused (ADR-009). This is
 * the only Foundation-family file that calls DriveApp, mirroring
 * FoundationDataStore.gs's "only file calling SpreadsheetApp" precedent
 * (docs/42 §15).
 *
 * ---- Authorization: the application is the boundary, Drive is defense-in-depth only ----
 * Per the approved architecture decision (docs/42 §6, "the single most
 * important property to design for and test explicitly in this batch"),
 * authorization always begins with this application, never with Drive's
 * own sharing state. `foundationGetPatientReports_()` and
 * `foundationDownloadReport_()` both take `patientId` only from an
 * already-verified session (ADR-002, enforced by the caller, same as
 * every other Foundation entity read). `foundationDownloadReport_()`
 * additionally accepts a client-supplied `recordId` —
 * `foundationGetReportById_()` independently verifies the fetched row's
 * own `patient_id` matches before any Drive call is ever made, the same
 * "unguessability is not itself an authorization boundary" discipline
 * `foundationGetConsultationEntryById_()` established
 * (docs/40-CONSULTATION-IDENTITY-STRATEGY.md Q3). The Drive file itself
 * is never left at an assumed default sharing state — every file this
 * application creates has its sharing explicitly set to private/no-access
 * (`foundationEnsureReportFilePrivate_()`) immediately after creation, a
 * verified, enforced property (see Stage 9's dedicated assertion), not
 * "we called `DriveApp.createFile()` and didn't set sharing" (docs/42 §6:
 * "a default is not the same as a verified guarantee"). Downloads never
 * hand the browser a Drive URL; the server fetches bytes inside its own
 * already-authorized execution and returns them base64-encoded.
 *
 ...
 * byte-structure sniffing) and its explicitly disclosed limitation,
 * including docs/42 §11's own specific open item: the exact behavior of
 * `Utilities.newBlob()`'s detection has not been verified against a live
 * Apps Script deployment in this environment — this file does not claim
 * stronger validation than actually exists.
 *
 * ---- Google Drive integration: one folder, record_id-named objects ----
 * Per docs/42 §5's recommendations: one fixed, private Drive folder,
 * never patient-partitioned; the Drive object is named using the
 * generated `record_id` alone (never the patient-supplied filename, and
 * never `patient_id` — see this file's own `foundationExtensionForMimeType_()`
 * call site below for why `patient_id` is deliberately excluded from the
 * object name too, the same "avoid another place patient_id could leak
 * into a name" reasoning docs/42 §5 already applied to folder structure).
 *
 * ---- Partial-write failure: Drive succeeds, Sheets fails ----
 * The platform's first entity whose lifecycle spans two independent
 * storage systems (docs/42 §1). `foundationCreateReport_()` writes Drive
 * first, then Sheets; if the Sheets write then fails, it rolls back by
 * trashing the just-created Drive file (preferred, per the approved
 * decision) before returning the standard generic error envelope. If the
 * rollback itself fails, the file is left in place as an accepted,
 * explicitly audit-logged orphan (`report_upload_orphaned_file`,
 * distinguishable from `report_upload_rolled_back`) rather than silently
 ...
 * category of gap. It reuses an already-existing Drive file in place —
 * the sharing-privacy enforcement above applies only to files this
 * application itself creates via the patient upload path; a file staff
 * placed in Drive by some other means remains staff's own placement
 * responsibility, not silently overridden by this wrapper.
 *
...
// One fixed folder, never patient-partitioned by subfolder (docs/42 §5).
 ...
 * §8, docs/42 §5). `mimeType`/`sizeBytes` are always the server-detected/
 * measured values, passed in by the caller after the real content-based
 * check.
 */
...
/**
 * Explicitly sets — never merely assumes — that a Drive file this
 * application just created has no sharing beyond the script's own owning
 * account. docs/42 §6: "a default is not the same as a verified
 * guarantee." Applied only to files this function's own caller creates
 * via `DriveApp.createFile()`; never applied to a file referenced by the
 * manually-run staff wrapper (that file's placement/permissions remain
 * staff's own responsibility — see this file's own header comment).
 */
function foundationEnsureReportFilePrivate_(driveFile) {
...
function foundationCreateReport_(input) {
  ...
  return withFoundationErrorHandling_(function () {
    ...
    // Named using record_id alone — never the patient-supplied filename,
    // and never patient_id (docs/42 §5's "avoid another place patient_id
    // could leak into a name," applied here to file naming, not just
    // folder structure).
    var driveObjectName = recordId + foundationExtensionForMimeType_(detectedMimeType);
    ...
    // Enforced, not assumed (docs/42 §6) — see foundationEnsureReportFilePrivate_()'s
    // own header comment.
    foundationEnsureReportFilePrivate_(driveFile);

    var record;
    try {
      record = foundationBuildReportRecord_(input, recordId, foundationNowIso_(), driveFile.getId(), detectedMimeType, bytes.length);
      foundationDsInsert_(FOUNDATION_REPORTS_SHEET_, FOUNDATION_REPORTS_COLUMNS_, record);
    } catch (sheetErr) {
      // Partial-write failure: Drive succeeded, Sheets failed (docs/42
      // §1/§15's named failure mode — the platform's first entity
      // spanning two independent storage systems). Prefer rollback by
      // trashing the orphaned Drive file, per the approved decision; if
      // rollback itself fails, leave the file in place as an accepted,
      // explicitly audit-logged orphan rather than silently losing track
      // of it.
      ...
      try {
        foundationLogAuditEvent_(
          rolledBack ? 'report_upload_rolled_back' : 'report_upload_orphaned_file',
          input.patient_id.trim(), input.uploaded_by.trim(),
          'record_id=' + recordId + ';drive_file_id=' + driveFile.getId() + ';reason=' + (sheetErr && sheetErr.message)
        );
      } catch (auditErr) {
        // Best-effort only — audit logging must never mask the real
        // failure being handled here.
      }
      throw sheetErr; // re-thrown so withFoundationErrorHandling_'s own catch returns the standard generic envelope
    }
 ...
 * it, and never alters its sharing (that remains staff's own placement
 * responsibility — see this file's own header comment for why this
 * differs from the patient upload path's explicit privacy enforcement).
 * Runs the same content-based type/size validation as the patient upload
 * route (foundationCreateReport_()) before writing any metadata. No
 * rollback logic applies here — unlike the upload path, this function
 * never creates a new Drive file, so a Sheets-write failure here orphans
 * nothing this application is responsible for cleaning up.
 */
